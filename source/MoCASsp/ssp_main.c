...
int main(int argc, char* argv[])
{
  ...
  //CID 276408 : UNCHECKED RETURN VALUE
    ...
    /* ==== Start the flood-on-touch watcher thread (detached, no behavior change otherwise) ==== */
    {
        ...
        if (pthread_create(&th, &attr, flood_on_touch_thread, NULL) != 0) {
            CcspTraceInfo(("Failed to create flood-on-touch thread: %s\n", strerror(errno)));
        } else {
            CcspTraceInfo(("Flood-on-touch ENABLED (touch %s to start, remove to stop)\n",
                           MOCA_FLOOD_TRIGGER_FILE));
        }
