...
log "INFO" "Running make for CcspMoCA_gtest.bin..."
...
if [ -f "./source/test/CcspMoCA_gtest.bin" ]; then
    log "INFO" "Running CcspMoCA_gtest.bin"
    ./source/test/CcspMoCA_gtest.bin
    log "INFO" "Completed Test Execution"
else
    log "ERROR" "CcspMoCA_gtest.bin does not exist, cannot run tests"
    exit 1
fi
